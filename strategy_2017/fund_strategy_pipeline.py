...
def find_default_validation_dir(input_dir: Path) -> Path | None:
    ...
    candidates = sorted(
        path
        for path in BASE_DIR.rglob(quarter_name)
...
def main() -> int:
    ...
    validation_dir = resolve_cli_path(args.validation_dir) if args.validation_dir else find_default_validation_dir(quarter_dir)
