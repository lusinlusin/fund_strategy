...
def find_default_index_path(filename: str) -> Path:
    candidates = [
        BASE_DIR / "akshare_index_data" / filename,
        BASE_DIR.parent / "akshare_index_data" / filename,
        BASE_DIR / filename,
    ...
    for path in candidates:
        if path.exists():
            return path
    return BASE_DIR / "akshare_index_data" / filename
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--hs300-path",
        default=str(find_default_index_path("sh000300.csv")),
        help="CSV file for the CSI 300 index.",
    )
    parser.add_argument(
        "--treasury-path",
        default=str(find_default_index_path("sh000012.csv")),
...
def run_backtest(
    ...
    effective_end_date = end_date if end_date is not None else nav_dates[-1]
    ...
    for signal_date in rebalance_dates:
        ...
        if execution_date is None or execution_date > effective_end_date:
            continue
    ...
    period_boundary_dates = execution_dates + [effective_end_date]
    ...
    benchmark_returns = build_benchmark_return_series(
    ...
    for position, current_date in enumerate(signal_dates):
        ...
        backtest_rows.append(
            {
                ...
                "next_signal_date": pd.Timestamp(next_signal_date).isoformat() if pd.notna(next_signal_date) else None,
        ...
        period_holdings.insert(2, "next_signal_date", pd.Timestamp(next_signal_date).isoformat() if pd.notna(next_signal_date) else None)
...
def main() -> int:
    args = parse_args()
    quarter_dir = resolve_cli_path(args.quarter_dir)
    output_dir = resolve_cli_path(args.output_dir)
    hs300_path = resolve_cli_path(args.hs300_path)
    treasury_path = resolve_cli_path(args.treasury_path)
